function processedData = intervalUniformer(rawData, rawAllDataBeforeFormat)
...
for i = 1:conditionsLength
    if(~isempty(rawData.liquidFlowRate)) % MATLABにはnullabaleが無く, 型ガードもないためemptyチェックが結構危ういが, 既存コードとの互換性のために致しかたなし.
        ...
        TimeTabledDataCell{i} = retime(TimeTabledDataCell{i},'regular','linear','SampleRate',rawData.measuredFreq); %おかしなデータは線形内挿.
