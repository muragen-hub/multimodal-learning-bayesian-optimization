function func = easyQiangXuBranch(moveMeanRange, moveMaxMinRange,moveSTDRange)
arguments
...
%QIANGXUBRANCH QiangXuさんの論文のbranchのさせ方を行う.  移動平均を抜いたもの. 
%移動最大最小値(移動レンジ), 移動標準偏差, 平均値からの偏差の絶対値の平均, 偏差平均を平均の代わりに使って求めた標準偏差
    function result = qiangXuBranchCore(rawData, ~)
        ...
        for idx=1:conditionLength
            ...
            tmp2 = cat(2,moveDiffOfMaxMin,movSTD,movError,movAbsoluteStandardDeviation); %ここ, 行べクトルか列ベクトルかで統一しなかったことの悪い点が出てる. cat(1,~~~)だと同じものを一つのシーケンスにしちゃう.
