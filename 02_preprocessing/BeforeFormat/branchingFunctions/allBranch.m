function func = allBranch(moveMeanRange, moveMaxMinRange,moveSTDRange)
arguments
...
%almostAllInput 考えられるほぼ全ての入力を与えてみる. 特徴量調査用. 
    function result = qiangXuBranchCore(rawData, ~)
        ...
        for idx=1:conditionLength
            ...
            %まとめ
            tmp2 = cat(2,WT1,WT2,WT3,WT4,WT5,WT6,WT7,WT8,moveMeanData,moveDiffOfMaxMin,movSTD,movError,movAbsoluteStandardDeviation,oneStepError,twoStepError,threeStepError,fourStepError,accumurate,logFeatures); %ここ, 行べクトルか列ベクトルかで統一しなかったことの悪い点が出てる. cat(1,~~~)だと同じものを一つのシーケンスにしちゃう.
